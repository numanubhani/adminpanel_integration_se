...
const InvitationTable = ({ data: initialData }) => {
  ...
  const filteredData = useMemo(() => {
    ...
    return initialData.filter((item) => {
      ...
      return item.email.toLowerCase().includes(searchStr);
    });
  ...
  return (
    <div className="w-full mx-auto">
        ...
        <table className="w-full">
          <thead>
            <tr className="bg-[#ceb46a] text-black">
              ...
              <th className="py-4 px-6 text-left">Joining Status</th>
              <th className="py-4 px-6 text-left">Actions</th>
            </tr>
          </thead>
          <tbody>
            {paginatedData.map((invitation, index) => (
              <tr
                key={invitation.id}
                className={
                  index % 2 === 0
              ...
              >
                <td className="py-4 px-6">{invitation.email}</td>
                <td className="py-4 px-6">{invitation.method}</td>
                <td className="py-4 px-6">{invitation.date}</td>
                <td className="py-4 px-6">
                  <span
                    className={`px-3 py-1 rounded-full text-sm ${
                      invitation.joining_status === "Joined"
                        ? "text-[#2D8E00] border border-[#2D8E00]"
                        : invitation.joining_status === "Pending"
                        ? "text-[#FFA929] border border-[#FFA929]"
                        : "text-[#EA4335] border border-[#EA4335]"
                    }`}
                  >
                    {invitation.joining_status}
                  </span>
                </td>
                <td className="py-4 px-6">{invitation.method}</td>
                <td className="py-4 px-6">
                  <div className="flex gap-2">
                    <button className="p-2 border border-[#EA4335] rounded">
                      <svg
                        width="16"
                        height="16"
                        viewBox="0 0 24 24"
      ...
      <div className="flex justify-between items-center mt-4">
        ...
        <div className="flex gap-2">
          <button
            className="px-4 py-1 border border-gray-300 text-[#667080] rounded flex items-center gap-1 disabled:opacity-50"
